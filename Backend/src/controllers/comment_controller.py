...
async def get_comments(post_id: str, category_id: str, db: Session):
    try:
        comments = db.query(Comment).filter(Comment.post_id == post_id, Comment.category_id == category_id).all()
        if not comments:
...
async def edit_comment(post_id: str, category_id: str, content: str, db: Session):
    try:
        comment = db.query(Comment).filter(Comment.post_id == post_id, Comment.category_id == category_id).first()
        if not comment:
...
async def vote_comment(request: VoteRequest, db: Session):
    try:
        comment = db.query(Comment).filter(Comment.post_id == request.post_id, Comment.category_id == request.category_id).first()
        if not comment:
            raise APIError(status_code=404, detail="Comment not found.")
