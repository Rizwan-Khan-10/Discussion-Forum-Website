function createCategorySection(categoryName, categoryId) {
    ...
    section.innerHTML = `
        <h2 class="text-xl font-semibold text-white mb-3 capitalize text-center bg-slate-950">${categoryName}</h2>
        <div class="flex gap-6 overflow-x-auto pb-2 hide-scrollbar"></div>
    `;
...
function addPostToCategory(categoryId, postData) {
    const category = document.querySelector(`#${categoryId} > div`);
    if (!category) return;
...
<div class="flex justify-between text-sm text-gray-300 mt-4 border-t border-gray-600 pt-3">
    ...
    category.appendChild(postCard);
}

async function fetchPopularPosts() {
    try {
        const response = await fetch('http://localhost:8000/post/Explore');
        if (!response.ok) {
